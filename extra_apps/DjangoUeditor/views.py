...
def UploadFile(request):
    ...
    if state == "SUCCESS":
        if action == "uploadscrawl":
            ...
            state = save_upload_file(file, os.path.join(OutputPath, OutputFile), action)

        url = urljoin(USettings.gSettings.MEDIA_URL, OutputPathFormat)
        if action != 'uploadvideo':
            print(url, upload_file_name)
            url = urljoin(USettings.gSettings.MEDIA_URL, OutputPathFormat) + '/' + upload_file_name
